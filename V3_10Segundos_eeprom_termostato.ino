...
NTPClient timeClient(ntpUDP, "europe.pool.ntp.org", 0, 300000);                 //--------------   0, 60000
int estado_boton_C_W = HIGH;                
int ultimoEstado_boton_C_W = HIGH;            
unsigned long tiempoUltimoCambio_boton_C_W = 0;    
long RetardoCambio_boton_C_W = 10000;               // Tiempo de pulsacion para entrar en configuracion
...
int Bot_mtbs = 3000;                   //tiempo entre comprobacion de nuevo mensajes
...
unsigned long tiempoUltimaLecturaTermostato = 0;    
long RetardoLecturaTermostato = 5000;
long Hister =2;
...
void MemTermostato(){              //-------------------Memoria Termostato --------------------

  if (C_Termostato != EEPROM.read(98)) EEPROM.write(98,C_Termostato);
  EEPROM.commit();
...
void interuptSensorPir() {                        //---------- Interupt Pir ---------------------
  ...
  if(sensorP == HIGH){
  //Serial.println("interuptSensorPir");//BOTON 1 D6
    sensorPirFlag = true;
  }
  return;
}
...
void handleNewMessages(int numNewMessages) {         // ---------- Telgram functions ------------
  for (int i=0; i<numNewMessages; i++) { 
    ...
    if (text == "/Termo") {
      contolAUX();
      bot->sendMessage(chat_id, "Calefaccion " + String(esTadoAux2));
    }
    if (text == "/Temp") {
      //Serial.end();
      //delay (500);//Retardo
    ...
    if (text == "/+1") {
      (C_Termostato)=(C_Termostato) + 1;
      if (C_Termostato <= 0){ (C_Termostato) = 1 ; bot->sendMessage(chat_id, "Minimo es 1º");}
      if (C_Termostato >= 100){ (C_Termostato) = 99 ; bot->sendMessage(chat_id, "Maximo es 99º");}
      bot->sendMessage(chat_id, "Termostato esta en : " + String(C_Termostato)+"º");
    }
    if (text == "/-1") {
      (C_Termostato)=(C_Termostato) - 1;
      if (C_Termostato <= 0){ (C_Termostato) = 1 ; bot->sendMessage(chat_id, "Minimo es 1º");}
    ...
    if (text == "/Mem") {
      ...
      if (C_Termostato >= 100){ (C_Termostato) = 99 ; bot->sendMessage(chat_id, "Maximo es 99º");}
      MemTermostato();
      C_Termostato=EEPROM.read(98);
      bot->sendMessage(chat_id, "Termostato Memorizado en : " + String(C_Termostato)+"º");
    }
    if (text == "/tecla" || text == "/Tecla") {
      String keyboardJson = "[[\"/Puerta\", \"/PP\", \"/Puerta2\", \"/Luz\", \"/Temp\"],[\"/Estado\",\"/Pir\", \"/AireOn\", \"/AireOff\", \"/Tecla2\"]]";
      bot->sendMessageWithReplyKeyboard(chat_id, "Teclado1", "", keyboardJson, true);
    }
    if (text == "/tecla2" || text == "/Tecla2") {
      String keyboardJson = "[[\"/Termo\", \"/+1\", \"/-1\", \"/+5\", \"/-5\"],[\"/Estado\",\"/Temp\", \"/CT\", \"/Mem\", \"/Tecla\"]]";
      bot->sendMessageWithReplyKeyboard(chat_id, "Teclado2", "", keyboardJson, true);
    }
    ...
    if (text == "/Ayuda" || text == "/ayuda") {
      ...
      welcome += "/Termo : Termostato 0-1\n";
      welcome += "/Luz : Luz 0-1\n";
      welcome += "/CT : Temperatura Ajuste Termostato\n";
      welcome += "/Mem : Memoria Termostato\n";
      welcome += "/Temp : Temperatura Ambiente\n";
      welcome += "/Estado : Indica el estado de Puerta,Sensor de Movimiento,Luz,Etc.\n";
      welcome += "/tecla : Teclado 1\n";
      welcome += "/tecla2 : Teclado 2\n";
...
void setup() {                  //------------------------ SETUP-------------------------------- 
  ...
  //ticker.attach(2.5, tick);
    ...
    C_Termostato=EEPROM.read(98);                 // ------------EEPROM Termostato---------------------
...
void loop() {
  ...
  if ((Chek_net2) >= vez_comprobacion){
   ...
   ultimoEstado_boton_C_W = Lectura_boton_C_W;            
     ...
     if (millis() > tiempoUltimaLecturaTermostato + RetardoLecturaTermostato)  {    //--------------Termostato--------------------
    sensors.requestTemperatures();   //Se envía el comando para leer la temperatura
    float temp= sensors.getTempCByIndex(0); //Se obtiene la temperatura en ºC
    if ( controlaux == 0 ){
    if (temp <= C_Termostato){
      digitalWrite(PIN_AUX, LOW);
      if (esTadoAux != String("Encendida")){bot->sendMessage(defaultChatId, "Calefaccion Encendida!");
      }
      esTadoAux =  String("Encendida");
    }
    if (temp >= C_Termostato + Hister){
      digitalWrite(PIN_AUX, HIGH);
      if (esTadoAux != String("Apagada")){bot->sendMessage(defaultChatId, "Calefaccion Apagada!");
      }
      esTadoAux =  String("Apagada");
    }
    }
    tiempoUltimaLecturaTermostato = millis(); 
}
}
