...
int controlluz = HIGH;
int controlaux = HIGH;
int PirHighLow = HIGH;       //estado inician sensores
String esTadoPir = "Pir Encendido";
String esTadoLuz = "Apagada";
String esTadoAux = "Apagado";
...
int Bot_mtbs = 1500;                   //tiempo entre comprobacion de nuevo mensajes
...
void contolAUX() {                                 //---------- Control Aux ---------------------
  if ( controlaux == HIGH ) {
    controlaux = LOW;
    esTadoAux =  String("Encendido");
    digitalWrite(PIN_AUX, LOW); 
  } else {
    controlaux = HIGH;
    esTadoAux =  String("Apagado");
...
void handleNewMessages(int numNewMessages) {         // ---------- Telgram functions ------------
  for (int i=0; i<numNewMessages; i++) { 
    ...
    if (text == "/Aux") {
      contolAUX();
      bot->sendMessage(chat_id, "Aux " + String(esTadoAux));
    }
    if (text == "/Temp") {
      //Serial.end();
      //delay (500);//Retardo
    ...
    if (text == "/tecla" || text == "/Tecla") {
      String keyboardJson = "[[\"/Puerta\", \"/PP\", \"/Aux\", \"/Luz\", \"/Temp\"],[\"/Puerta2\",\"/Pir\", \"/AireOn\", \"/AireOff\", \"/Estado\"]]";
    ...
    if (text == "/Ayuda" || text == "/ayuda") {
      ...
      welcome += "/Estado : Indica el estado de Puerta,Sensor de Movimiento,Luz y Aux\n";
...
void loop() {
  ...
  if (millis() > Chek_lasttime + Chek_net) {    //-------------Chec_lasttime--------------------
    ...
    bot->sendMessage(defaultChatId, "Fallo de Reloj : Reinicio!");   //mensaje Restart
    delay (10000);//Retardo
    ESP.restart();
    }
    }
