...
#define LED_ESTADO D0     //LED ESTADO Y PIR ON-OFF  D8
...
long RetardoCambio_boton_C_W = 0;               // Tiempo de pulsacion para entrar en configuracion
...
//AireOn Frio
uint16_t  rawData3[115] = {3078,9106,530,1558,508,486,508,486,508,486,508,486,508,486,508,486,508,514,526,488,508,1556,508,486,508,486,508,486,508,486,508,486,508,516,528,1556,508,1556,508,1556,508,1556,508,1556,508,1556,508,1556,508,1586,530,1556,508,486,508,486,508,486,508,486,508,486,508,486,508,514,526,488,508,486,508,486,508,486,508,1556,508,486,508,1556,508,516,528,1556,508,486,508,486,508,486,508,1556,508,486,508,486,508,514,526,488,508,486,508,486,508,486,508,1556,508,1556,508,1556,508,1556,524};
...
void aire_frio() {                                //---------- Aire ON --------------------- 
  irsend.sendRaw(rawData3, 115, 38);  // Send a raw data capture at 38kHz.
}
...
void ejecutaNtc()  {                                //--------------Temperatura ntc -------------
  int val = 0;
  for(int i = 0; i < 20; i++) {
    val += analogRead(A0);
    delay(1);
  ...
  R_NTC = log(R_NTC);
  double TempN = 1 / (0.001129148 + (0.000234125 + (0.0000000876741 * R_NTC * R_NTC ))* R_NTC );
  tempNtc = TempN - 273.15;          
 }
void handleNewMessages(int numNewMessages) {         // ---------- Telgram functions ------------
  for (int i=0; i<numNewMessages; i++) { 
    ...
    if (timeClient.getEpochTime()< date2)  {
     //bot->sendChatAction(chat_id, "find_location"); //"Parece que la hora esta mal intento sincronizar"
    ...
    if (text == "/AireF") {
      aire_frio();
      bot->sendMessage(chat_id, "Aire ON 21º Frio");
    }
    if (text == "/AireOff") {
      aire_off();
    ...
    if (text == "/Temp") {
      ...
      float temp= sensors.getTempCByIndex(0); //Se obtiene la temperatura en ºC
      ejecutaNtc();
      bot->sendMessage(chat_id, "temperatura Ext "+ String(temp)+"º"+ ".\n" + "temperatura Int "+ String(tempNtc)+"º"); 
      //Serial.begin(115200); 
    }
    if (text == "/Ntc") {
      bot->sendChatAction(chat_id, "typing");
       ejecutaNtc();   
      bot->sendMessage(chat_id, "temperatura Ntc "+ String(tempNtc)+"º"); 
      //Serial.begin(115200); 
    }
    ...
    if (text == "/Ayuda" || text == "/ayuda") {
      ...
      welcome += "/AireC : Enciende el AA en Calor\n";
      welcome += "/AireF : Enciende el AA en Frio\n";
      welcome += "/AireOff : Apaga el AA\n";
      welcome += "/Aux : Aux 0-1\n";
      welcome += "/Luz : Luz 0-1\n";
      welcome += "/Temp : Temperatura Ambiente\n";
      welcome += "/ip : ip publica\n";
